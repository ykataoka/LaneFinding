...
def lane_preprocess(img):
    ...
    try:
        ...
        #        colorY_bin = colorH_bin & colorS_bin
        ...
        comb1[((colorS_bin == 1) | (colorR_bin == 1) | (colorH_bin == 1)) &
              (dir_bin == 1)] = 1
        comb2 = np.zeros_like(dir_bin)
        comb2[((colorS_bin == 1) | (colorR_bin == 1)) &
...
#              | ((colorH_bin == 1) & (magXY_bin == 1))] = 1
        ...
        L_Line.best_fit = np.sum(L_Line.current_fit, axis=0) \
                          / len(L_Line.current_fit)
        R_Line.best_fit = np.sum(R_Line.current_fit, axis=0) \
                          / len(R_Line.current_fit)
...
#    img_files = glob.glob('test_challenge2/*')
